    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 1000,
          system: `You are an AI assistant embedded in a Stock Exchange Simulator built with C++ (matching engine), Python (RAG/AI), and React frontend. 
The exchange uses a price-time priority matching algorithm. Key concepts: Order Book, Matching Engine, Buy/Sell Orders, Trade History, Market Analytics.
Stocks: RELIANCE, TCS, INFY (NSE-style Indian market).
Answer concisely and educationally in 2-4 sentences. Be accurate about stock exchange mechanics.`,
          messages: [{ role: "user", content: q }],
        }),
      });
      const data = await res.json();
      return data?.content?.[0]?.text || "I couldn't retrieve an answer right now. Please try again.";
    } catch {
      return "Demo mode: The AI backend isn't reachable. In production, this connects to your RAG pipeline (FAISS + Gemini) via POST /ask.";
    }
  }
